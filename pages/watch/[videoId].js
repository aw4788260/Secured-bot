...
export default function WatchPage() {
    ...
    const hlsRef = useRef(null); // لتخزين نسخة HLS
    ...
    useEffect(() => {
        ...
        if (window.Hls.isSupported()) {
            // تدمير النسخة القديمة إن وجدت
            if (hlsRef.current) hlsRef.current.destroy();

            hls = new window.Hls({
                maxBufferLength: 30,
                enableWorker: true,
                // [التعديل الجوهري] هذا يحل مشاكل CORS البسيطة في WebView
            ...
            hls.on(window.Hls.Events.ERROR, (event, data) => {
                if (data.fatal) {
                    // إذا حدث خطأ في الشبكة، سنحاول إعادة التحميل
    ...
    return (
        <div className="page-container">
            <Head>
                ...
                {/* [هام] إخفاء الريفيرر يساعد في تخطي قيود جوجل */}
            ...
            <div className="video-wrapper">
                ...
                {/* زر الجودة المخصص (يظهر فقط لو فيه جودات) */}
                {qualities.length > 0 && (
