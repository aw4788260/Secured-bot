...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      // 2. جلب المدرسين بشكل منفصل لربطهم برمجياً
      const { data: teachers } = await supabase.from('teachers').select('id, name');

      // دمج البيانات برمجياً لتوفر كائن متوافق مع الفرونت إند (p.teachers?.name)
      const enrichedPrizes = prizes ? prizes.map(prize => ({
          ...prize,
          teachers: teachers?.find(t => t.id === prize.teacher_id) || null
      })) : [];

      // 3. 🎯 جلب جدول الفائزين (المشاركات) بالكامل مع اسم الجائزة من جدول الجوائز
      const { data: winners, error: winnersError } = await supabase
        .from('wheel_spins')
        .select('*, wheel_prizes(title, type)')
        .order('id', { ascending: false }); // من الأحدث للأقدم

      // 4. جلب الحالة العامة لتفعيل العجلة
  ...
  if (req.method === 'POST') {
    ...
    try {
      // الإجراء الجديد 🎯: تغيير حالة تفعيل العجلة بالكامل
      ...
      if (action === 'activate_campaign') {
         ...
         return res.status(200).json({ success: true, message: `تم تفعيل الحملة! يتوفر الآن ${ticketsPool.length} جائزة جاهزة في الصندوق.` });
      }
