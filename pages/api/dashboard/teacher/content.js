...
export default async (req, res) => {
  ...
  try {
    ...
    if (action === 'create') {
      ...
      if (type === 'videos') {
          const isBunnyUpload = !!insertData.bunny_video_id;
          const videoUrl = insertData.url || insertData.youtube_video_id;

          // يجب توفر إما bunny_video_id أو رابط يوتيوب
          if (!isBunnyUpload && !videoUrl) {
              return res.status(400).json({ error: 'رابط الفيديو أو معرف Bunny مطلوب.' });
          }
          ...
          // 🚨 المدة إلزامية فقط لروابط يوتيوب — فيديوهات Bunny تحصل على المدة من confirm-upload
          if (!isBunnyUpload) {
              if (!insertData.duration || insertData.duration.trim() === '' || insertData.duration === '00:00') {
                  return res.status(400).json({ error: 'إرسال مدة الفيديو إلزامي عند استخدام رابط يوتيوب.' });
              }
          } else {
              // لا نطلب duration من الكلاينت لفيديوهات Bunny
              // (duration_seconds يُحفظ مسبقاً في confirm-upload من bunnyVideo.length)
              if (insertData.duration === undefined) delete insertData.duration;
          }
      }
    ...
    if (action === 'update') {
       ...
       if (type === 'videos') {
           ...
           // 🚨 التأكد من أن المدة المرسلة للتحديث صالحة (إذا أُرسلت — اختيارية لـ Bunny)
           if (updates.duration !== undefined && updates.duration !== null) {
               if (updates.duration.trim() === '' || updates.duration === '00:00') {
                   return res.status(400).json({ error: 'تحديث مدة الفيديو لا يمكن أن يكون فارغاً أو أصفاراً.' });
               }
           }
       }
