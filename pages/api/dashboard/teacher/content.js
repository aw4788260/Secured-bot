...
export default async (req, res) => {
  ...
  // 1. التحقق من الصلاحية (باستخدام المساعد الجديد للداشبورد)
  const user = await requireTeacherOrAdmin(req, res);
  if (!user) return; // الرد بالخطأ تم التعامل معه داخل الدالة المساعدة

  // 2. تجهيز متغيرات الصلاحية لتتوافق مع الكود الأصلي
  // هذا يضمن أن teacherId متاح لباقي الدالة كما كان سابقاً
  const auth = {
  ...
  try {
    ...
    if (action === 'create') {
      ...
      // إدارة الصلاحيات (Grant Access to Creator)
      if (type === 'courses' && newItem) {
          try {
            ...
            // إضافة حساب الأدمن الرئيسي للمدرس (إذا كان مختلفاً عن المستخدم الحالي)
            // هذا يحدث إذا كان المساعد هو من يرفع الكورس
