...
const editMessage = async (chatId, messageId, text, reply_markup = null, parse_mode = null, protect_content = false) => {
    ...
    const createPayload = (isCaption = false) => {
        const payload = {
            chat_id: chatId,
            message_id: messageId,
            [isCaption ? 'caption' : 'text']: processedText,
            protect_content: protect_content
        };
        if (reply_markup) payload.reply_markup = reply_markup;
    ...
    try {
        // --- المحاولة 1: تعديل النص (editMessageText) ---
        await axios.post(`${TELEGRAM_API}/editMessageText`, createPayload(false));
        ...
        if (textError.response && textError.response.data) {
            ...
            if (desc.includes("no text in the message to edit") || desc.includes("message can't be edited")) {
                ...
                console.warn(`editMessageText failed. Attempting editMessageCaption...`);
                try {
                    // --- المحاولة 2: تعديل الكابشن (editMessageCaption) ---
                    await axios.post(`${TELEGRAM_API}/editMessageCaption`, createPayload(true));
                    ...
                    if (captionDesc.includes("can't parse entities")) {
                        ...
                        plainCaptionPayload.caption = text; // النص الأصلي غير المهيأ
                        try {
                            await axios.post(`${TELEGRAM_API}/editMessageCaption`, plainCaptionPayload);
                            return; // نجاح
                        } catch (plainCaptionError) {
                             console.error(`[Fallback] Plain caption failed:`, plainCaptionError.response?.data || plainCaptionError.message);
                             throw plainCaptionError; // ارمي الخطأ الأخير
                        }
                    }
                    throw captionError; // ارمي خطأ الكابشن
                }
                return; // (نجاح تعديل الكابشن)
            }

            // (فشل تعديل النص بسبب التنسيق)
            if (desc.includes("can't parse entities")) {
                ...
                const retryPayload = createPayload(false);
                delete retryPayload.parse_mode;
                retryPayload.text = text; // النص الأصلي
                try {
            ...
            console.error(`Failed to edit message ${chatId}:${messageId}:`, textError.message);
            throw textError;
        }
    }
};
...
const loadQuestionsForEditSession = async (chatId, messageId, stateData) => {
    ...
    if (questions.length === 0) {
        ...
        await setUserState(chatId, 'awaiting_question_edit', { ...stateData, questions: [], current_index: 0, current_edit_message_id: messageId });
...
const displayQuestionForEdit = async (chatId, messageId, stateData) => {
    ...
    try {
        ...
        // (4. تحديث الحالة بالـ ID الجديد للرسالة)
...
// 4. إدارة المحتوى (الإضافة - [ ✅ تعديل: للسؤال عن السعر ])
      ...
      // (الكود الجديد - للاستبدال)
      // (4. تفعيل زر تعديل الأسئلة - [✅ معدل بمنطق الإغلاق وبدون نافذة منبثقة])
      if (command.startsWith('exam_edit_questions_')) {
         ...
         if (count > 0) {
              ...
              // 1. (تم حذف النافذة المنبثقة "answerCallbackQuery")
              
              // 2. تعديل الرسالة نفسها لتأكيد الإغلاق (كما هو)
              const { data: exam } = await supabase.from('exams').select('subject_id').eq('id', examId).single();
              const kbd = { inline_keyboard: [
                  [{ text: '🔙 رجوع (لقائمة الامتحانات)', callback_data: `content_nav_exams_for_subject_${exam ? exam.subject_id : ''}` }]
              ]};
              await editMessage(chatId, messageId, `🔒 هذا الامتحان مغلق للتعديلات.\n\nالسبب: قام ${count} طالب بإنهاء هذا الامتحان بالفعل.\n\n(لحماية نزاهة الدرجات، لا يمكن تعديل الأسئلة بعد أن يتم حل الامتحان. يمكنك حذف الامتحان وإنشاء واحد جديد.)`, kbd);
         ...
         await loadQuestionsForEditSession(chatId, messageId, { 
             ...
             message_id: messageId, 
             exam_id: examId,
             subject_id: stateData.current_subject_id 
         });
         return res.status(200).send('OK');
      }
...
// (1. حالات المستخدم العادي - إرسال صورة/نص)
      ...
      if (user.is_admin && currentState) {
        ...
        switch (currentState) {
            ...
            // (الكود التالي يضاف *داخل* الـ switch، قبل case 'awaiting_user_ids')
          ...
          case 'awaiting_replacement_question_poll':
            ...
            if (!message.poll) {
                await editMessage(chatId, stateData.current_edit_message_id, 'الرجاء إرسال "Poll" (استفتاء) فقط، أو /cancel');
                return res.status(200).send('OK');
            }
          ...
          case 'awaiting_new_question_end':
            ...
            // (تحديد الرسالة التي سيتم تعديلها أو الرد عليها)
            // (في حالة "الإنشاء" نستخدم message_id الأصلي، في حالة "التعديل" نستخدم ID رسالة السؤال)
            ...
            if (message.poll) {
                ...
                if (poll.type !== 'quiz' || poll.correct_option_id === null || typeof poll.correct_option_id === 'undefined') {
                    await editMessage(chatId, target_message_id, `❌ خطأ: الـ Poll ليس Quiz أو لم يتم "حلّه".\n\n(جاري انتظار السؤال التالي أو /done)`);
                    return res.status(200).send('OK');
                }
                ...
                try {
                    ...
                    if (currentState === 'awaiting_new_question_after') {
                        await editMessage(chatId, target_message_id, '⚙️ جاري إضافة السؤال...');
                        ...
                        await saveParsedQuestions(parsedPoll, stateData, chatId, after_sort_order + 1);
                        ...
                        await saveParsedQuestions(parsedPoll, stateData, chatId, newSortOrder);
                         ...
                         await editMessage(chatId, target_message_id, `✅ تم حفظ ${newSortOrder} سؤال.
...
أرسل السؤال التالي (صورة ثم Poll، أو Poll مباشرة)، أو /done للانتهاء.`);
                    ...
                    // (إعادة تحميل الجلسة للحالات الأخرى)
                    await answerCallbackQuery(callback_query ? callback_query.id : '', { text: `✅ تم الحفظ ${wasImageSaved ? '🖼️' : ''}` });
                    await loadQuestionsForEditSession(chatId, target_message_id, { ...stateData, sticky_image_file_id: null });

                } catch (err) {
                    await editMessage(chatId, target_message_id, `حدث خطأ فادح: ${err.message}`);
                    await setUserState(userId, null, null);
                }
            ...
            // --- 3. التعامل مع النص (التنسيق الجديد) ---
                ...
                try {
                    ...
                    if (parsedQuestions.length === 0) {
                        await editMessage(chatId, target_message_id, '❌ خطأ: لم أتمكن من فهم التنسيق النصي.\n\nاستخدم "---" للفصل بين الأسئلة.\n\n(جاري انتظار السؤال التالي أو /done)');
                        return res.status(200).send('OK');
                    }

                    if (currentState === 'awaiting_exam_questions') {
                        await editMessage(chatId, target_message_id, `⚙️ تم العثور على ${parsedQuestions.length} سؤال نصي، جاري حفظهم...`);
...
أرسل المزيد (صورة ثم Poll، أو نص)، أو /done.`);
                    
                    } else {
                        await editMessage(chatId, target_message_id, 'لتعديل الأسئلة (الإضافة)، الرجاء إرسال (Poll) واحد فقط، أو /cancel.');
                    }

                } catch (err) {
                    await editMessage(chatId, target_message_id, `حدث خطأ أثناء تحليل النص: ${err.message}`);
                }
