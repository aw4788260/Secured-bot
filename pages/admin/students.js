...
export default function StudentsPage() {
  ...
  // --- منطق الفلتر (Modal Logic) ---
  const openFilterModal = () => {
      setTempFilters(activeFilters); // نسخ الفلاتر الحالية للتعديل عليها
  ...
  // العمليات الجماعية
  const toggleSelectAll = (e) => setSelectedUsers(e.target.checked ? currentTableData.map(u => u.id) : []);
  ...
  return (
    <AdminLayout title="إدارة الطلاب">
      ...
      <div className="controls-container">
          <input className="search-input" placeholder="🔍 بحث (اسم، هاتف، ID)..." value={searchTerm} onChange={e=>setSearchTerm(e.target.value)} />
          ...
          <div className="modal-overlay" onClick={() => setShowFilterModal(false)}>
              <div className="modal-box filter-modal" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="modal-footer" style={{justifyContent: 'space-between'}}>
                      <button className="cancel-btn danger-text" onClick={clearFilters}>مسح الفلاتر 🗑️</button>
                      <button className="confirm-btn" onClick={applyFilters}>
                          عرض النتائج ({tempFilters.courses.length + tempFilters.subjects.length}) ✅
                      </button>
                  </div>
              </div>
          </div>
                              ...
                              <div className="subs-grid">
                                  <div className="sub-column">
                                      ...
                                      {userSubs.courses.map(c => (
                                          <div key={c.course_id} className="sub-chip"><span>{c.courses?.title}</span><button onClick={() => showConfirm('سحب؟', () => runApiCall('revoke_access', { userId: viewUser.id, courseId: c.course_id }))}>✕</button></div>
                                      ))}
                                  </div>
                                  <div className="sub-column">
                                      <h5>📄 المواد الفردية</h5>
                                      {userSubs.subjects.map(s => (
                                          <div key={s.subject_id} className="sub-chip"><span>{s.subjects?.title}</span><button onClick={() => showConfirm('سحب؟', () => runApiCall('revoke_access', { userId: viewUser.id, subjectId: s.subject_id }))}>✕</button></div>
                                      ))}
                                  </div>
                              </div>
                          )}
      ...
      <style jsx>{`
        ...
        .pagination { display: flex; justify-content: center; gap: 15px; margin-top: 25px; color: #94a3b8; padding-bottom: 50px; }
