...
export default function WheelManager() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // جلب المدرسين أولاً (لقائمة الاختيار الخاصة بالكوبونات)
  ...
  const openForm = (prize = null) => {
    if (prize) {
      ...
      setFormData({
        ...
        color: '#38bdf8', teacher_id: '', is_active: true
      });
    }
    setShowModal(true);
  };
  ...
  return (
    <SuperLayout title="إدارة عجلة الحظ">
      ...
      <div className="page-header">
        <div>
          <h1>🎡 إدارة عجلة الحظ</h1>
          <p>تجهيز الجوائز وتفعيل السحب للطلاب</p>
        </div>
        <div className="header-actions">
          ...
          <button className="btn success" onClick={() => showConfirm('هل أنت متأكد من تفعيل الحملة؟ سيتم خلط الصندوق بناءً على الجوائز المفعلة.', () => executeAction('activate_campaign'))}>
            🚀 تفعيل الحملة (Shuffle)
          </button>
        </div>
      </div>
      ...
      <div className="stats-grid">
         <div className="stat-card">
            <h3>🎁 الجوائز المُعرفة</h3>
            <div className="val">{prizes.length}</div>
         </div>
         <div className="stat-card">
            <h3>🎟️ تذاكر في الصندوق</h3>
      ...
      <div className="panel">
        <div className="panel-header">
           <h2>قائمة الجوائز (الكتالوج)</h2>
           <button className="btn primary" onClick={() => openForm()}>➕ إضافة جائزة</button>
        </div>
        
        {loading && <div style={{padding:'20px', textAlign:'center', color:'#38bdf8'}}>جاري التحميل...</div>}
              ...
              <tbody>
                {prizes.map(p => (
                  <tr key={p.id}>
                    ...
                    <td>{p.is_active ? '✅ مفعل' : '❌ معطل'}</td>
                    <td>
                      <div className="actions-cell">
                         <button className="icon-btn edit" onClick={() => openForm(p)}>✏️</button>
                         <button className="icon-btn delete" onClick={() => showConfirm('تأكيد حذف الجائزة؟', () => executeAction('delete_prize', { id: p.id }))}>🗑️</button>
                      </div>
                    </td>
                  </tr>
                ))}
                {prizes.length === 0 && <tr><td colSpan="7" style={{textAlign:'center', padding:'30px'}}>لا توجد جوائز، قم بإضافة واحدة.</td></tr>}
              </tbody>
            </table>
          </div>
        )}
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
           <div className="modal-box" onClick={e => e.stopPropagation()}>
              <div className="modal-head">
                 <h3>{formData.id ? 'تعديل الجائزة' : 'إضافة جائزة جديدة'}</h3>
                 <button onClick={() => setShowModal(false)} className="close-btn">✕</button>
              </div>
              <form onSubmit={handleSave} className="modal-body">
                 
                 <div className="form-group">
                    <label>الاسم (يظهر في العجلة)</label>
                    <input className="input" required value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="مثال: خصم 50% / آيفون / حظ أوفر" />
                 </div>
                 ...
                 <div className="grid-2">
                    <div className="form-group">
                        ...
                        <select className="input" value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})}>
                            ...
                            <option value="material">🎁 مادية (يتم تسليمها يدوياً)</option>
                            <option value="nothing">😢 حظ أوفر (لا شيء)</option>
                        </select>
                    </div>
                    <div className="form-group">
                        <label>المخزون الإجمالي (الاحتمالية)</label>
                        <input className="input" type="number" required min="1" value={formData.total_stock} onChange={e => setFormData({...formData, total_stock: e.target.value})} />
                    </div>
                 </div>

                 {formData.type === 'coupon' && (
                     <div className="coupon-box">
                         <h4>⚙️ إعدادات الكوبون</h4>
                         <div className="grid-2">
                             <div className="form-group">
                 ...
                 <div className="grid-2 mt-2">
                     <div className="form-group">
                         <label>لون القطعة في العجلة</label>
                 ...
                 <div className="modal-footer">
                    ...
                    <button type="submit" className="btn primary">حفظ الجائزة 💾</button>
                 </div>
              </form>
           </div>
        </div>
          ...
          <div className="modal-overlay">
              <div className="modal-box confirm-box">
                  ...
                  <div className="modal-footer centered">
                      ...
                      <button className="btn success" onClick={confirmData.action}>نعم، متأكد</button>
                  </div>
              </div>
          </div>
      )}
      ...
      <style jsx>{`
        ...
        .btn.success { background: #22c55e; color: #0f172a; } .btn.success:hover { background: #16a34a; }
        .btn.warning { background: transparent; border: 1px solid #f59e0b; color: #fcd34d; } .btn.warning:hover { background: rgba(245, 158, 11, 0.1); }
        ...
        .stat-card h3 { margin: 0 0 10px 0; color: #94a3b8; font-size: 1rem; font-weight: normal; }
        ...
        .stat-card p { margin: 5px 0 0; color: #64748b; font-size: 0.85rem; }
        ...
        th { padding: 15px; color: #94a3b8; font-size: 0.9rem; background: #1e293b; border-bottom: 1px solid #334155; }
        td { padding: 15px; border-bottom: 1px solid #334155; color: #cbd5e1; vertical-align: middle; }
        
        .color-box { width: 30px; height: 30px; border-radius: 6px; border: 2px solid #334155; }
        .sub-txt { font-size: 0.85em; color: #94a3b8; margin-top: 4px; }
        .stock-badge { background: rgba(56, 189, 248, 0.1); color: #38bdf8; padding: 4px 12px; border-radius: 20px; font-weight: bold; font-family: monospace; }
        
        .actions-cell { display: flex; gap: 8px; }
        .icon-btn { background: rgba(255,255,255,0.05); border: none; width: 35px; height: 35px; border-radius: 8px; cursor: pointer; transition: 0.2s; }
        ...
        .modal-box { background: #0f172a; width: 95%; max-width: 600px; border-radius: 16px; border: 1px solid #334155; overflow: hidden; animation: popIn 0.3s; }
        ...
        .input { width: 100%; padding: 12px; background: #1e293b; border: 1px solid #475569; border-radius: 8px; color: white; outline: none; }
        ...
        .coupon-box h4 { margin: 0 0 15px 0; color: #38bdf8; font-size: 0.95rem; }
