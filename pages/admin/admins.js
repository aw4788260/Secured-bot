...
export default function AdminsPage() {
  ...
  const handleDemote = (admin) => {
      showConfirm(`هل أنت متأكد من إزالة صلاحية الإشراف عن ${admin.first_name}؟`, () => {
  ...
  return (
    <AdminLayout title="المشرفين">
      <div className={`toast ${toast.show ? 'show' : ''} ${toast.type}`}>{toast.message}</div>

      <div className="page-header">
          <div>
            <h1>👮 إدارة فريق العمل</h1>
            <p className="sub-head">إدارة صلاحيات الوصول للوحة التحكم</p>
          </div>
          {isMainAdmin && (
              <button onClick={handlePromote} className="add-btn">➕ إضافة مشرف</button>
          )}
      </div>
      ...
      <div className="table-box">
          <table className="admin-table">
              ...
              <tbody>
                  ...
                  {admins.length === 0 && !loading && <tr><td colSpan="6" style={{textAlign:'center', padding:'20px'}}>لا يوجد مشرفين</td></tr>}
              </tbody>
          </table>
      </div>
      ...
      <style jsx>{`
        ...
        .admin-table th { background: #0f172a; padding: 15px; color: #94a3b8; border-bottom: 1px solid #334155; text-align: right; }
        ...
        .web-tag.inactive { background: rgba(148, 163, 184, 0.15); color: #94a3b8; border: 1px dashed #64748b; }

        .role-badge { padding: 4px 10px; border-radius: 20px; font-size: 0.85em; font-weight: bold; }
        ...
        .act-btn { width: 34px; height: 34px; border-radius: 8px; border: none; cursor: pointer; display: flex; justify-content: center; align-items: center; font-size: 1.1em; transition: 0.2s; }
        .act-btn.web { background: #3b82f6; color: white; } .act-btn.web:hover { background: #2563eb; }
        .act-btn.demote { background: #ef4444; color: white; } .act-btn.demote:hover { background: #dc2626; }
