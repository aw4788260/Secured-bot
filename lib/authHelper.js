...
export async function checkUserAccess(userId, videoId = null, pdfId = null, examId = null, deviceId = null) {
  ...
  try {
    ...
    if (videoId || pdfId) {
      ...
      if (videoId) {
          // إذا كان فيديو، نبحث في جدول الفيديوهات
          const { data } = await supabase.from('videos').select('chapter_id').eq('id', videoId).maybeSingle();
          if(data) chapterId = data.chapter_id;
      } else if (pdfId) {
          // [جديد] إذا كان PDF، نبحث في جدول pdfs
          const { data } = await supabase.from('pdfs').select('chapter_id').eq('id', pdfId).maybeSingle();
          if(data) chapterId = data.chapter_id;
      }
