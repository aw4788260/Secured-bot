...
import { APP_DOMAIN } from './config'; // ✅ 1. استيراد الدومين من ملف الإعدادات

// ✅ 2. استخدام المتغير المستورد بدلاً من الكتابة اليدوية
...
export async function checkUserAccess(req, resourceId = null, resourceType = null) {
  ...
  // ✅ التعديل الأمني الجديد:
  // بدلاً من التحقق من مجرد وجود الهيدر، نقوم بمقارنته مع السر المخزن في السيرفر
  const incomingSecret = req.headers['x-app-secret'];
  const validSecret = process.env.APP_SECRET; 

  // الشرط: الطلب لا يحتوي على Referer (غالباً تطبيق) + السر مطابق تماماً
  const isAppRequest = (!referer && incomingSecret && incomingSecret === validSecret);

  const isSourceValid = 
      isAppRequest || // 👈 السماح للتطبيق فقط إذا كان السر مطابقاً
      referer.includes(host) || 
      referer.includes('telegram') || 
      ALLOWED_DOMAINS.some(domain => referer.includes(domain));

  if (!isSourceValid) {
      errLog(`⛔ Blocked Referer: "${referer}" (Headers: SecretValid=${isAppRequest ? 'Yes' : 'No'})`);
      return false; 
  }
