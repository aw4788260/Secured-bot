...
export async function checkUserAccess(req, resourceId = null, resourceType = null) {
  ...
  try {
      ...
      if (cachedTokenInfo && cachedTokenInfo.expiry > currentTime) {
          ...
          log(`App Check Verified successfully (FROM CACHE)`); // يمكنك إيقاف هذه الطباعة لتقليل اللوجات
      } else {
          try {
              ...
              log(`App Check Verified successfully for app: ${appCheckClaims.app_id} (CACHED)`);
